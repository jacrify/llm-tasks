...
function findActiveTaskForParsedLine(
    ...
    if (parsed.sessionId) {
        return activeTasks.find(t => t.tmuxSession === parsed.sessionId);
    }
...
describe("findActiveTaskForParsedLine", () => {
    const activeTasks: TaskRecord[] = [
        {
            ...
            tmuxSession: "llm-2026-04-14_143022_task-alpha",
        ...
        },
        {
            ...
            tmuxSession: "llm-2026-04-14_150000_task-beta",
            sourceFile: "notes/test.md",
            sourceLine: 10,
            taskText: "Task beta",
            agentId: "pi",
    ...
    it("finds active task by session ID", () => {
        const parsed = parseTaskLine(
            "- ⏳ Task alpha <!-- llm:llm-2026-04-14_143022_task-alpha -->"
        )!;
    ...
    it("returns undefined when no match found", () => {
        const parsed = parseTaskLine("- ⏳ Unknown task <!-- llm:llm-unknown -->");
        expect(parsed).not.toBeNull();
        const result = findActiveTaskForParsedLine(parsed!, activeTasks);
